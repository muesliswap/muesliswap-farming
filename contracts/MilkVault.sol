...
contract MilkVault is Ownable, Pausable {
    ...
    IERC20 public immutable token; // Milk token
    IERC20 public immutable receiptToken; // Spoon token

    IMasterFarmer public immutable masterchef;
    ...
    /**
     * @notice Constructor
     * @param _token: Milk token contract
     * @param _receiptToken: Spoon token contract
     * @param _masterchef: MasterFarmer contract
    ...
    function harvest() external notContract whenNotPaused {
        IMasterFarmer(masterchef).leaveStaking(0);
    ...
    function emergencyWithdraw() external onlyAdmin {
        IMasterFarmer(masterchef).emergencyWithdraw(0);
    }
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = IMasterFarmer(masterchef).pendingCake(0, address(this));
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            IMasterFarmer(masterchef).leaveStaking(balWithdraw);
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = IMasterFarmer(masterchef).userInfo(0, address(this));
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IMasterFarmer(masterchef).enterStaking(bal);
        }
    }
